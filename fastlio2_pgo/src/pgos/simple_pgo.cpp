...
bool SimplePGO::addKeyPose(const CloudWithPose &cloud_with_pose)
{
    ...
    // Add the pose-graph initial value and factors when GTSAM is available.
#ifdef FASTLIO2_HAS_GTSAM
...
void SimplePGO::searchForLoopPairs()
{
    ...
    const double fitness_score = m_icp.getFitnessScore();
    const M4F loop_transform = m_icp.getFinalTransformation();
    if (!m_icp.hasConverged() || !std::isfinite(fitness_score) ||
        !loop_transform.allFinite() || fitness_score > m_config.loop_score_tresh)
        return;

    LoopPair one_pair;
    one_pair.source_id = cur_idx;
    one_pair.target_id = loop_idx;
    one_pair.score = fitness_score;
...
{
#ifndef FASTLIO2_HAS_GTSAM
    // The LIO poses are already stored as global poses. Mapping and map saving
    // remain functional without an optional pose-graph backend.
    m_cache_pairs.clear();
    return;
#else
