...
public:
    LIONode() : Node("lio_node")
    {
        ...
        m_imu_sub = create_subscription<sensor_msgs::msg::Imu>(
            m_node_config.imu_topic, rclcpp::SensorDataQoS().keep_last(200),
            std::bind(&LIONode::imuCB, this, std::placeholders::_1));
        m_lidar_sub = create_subscription<sensor_msgs::msg::PointCloud2>(
            m_node_config.lidar_topic, rclcpp::SensorDataQoS().keep_last(10),
        ...
        RCLCPP_INFO(
            get_logger(),
            "FAST-LIO2 Ouster input ready: points=%s imu=%s output=%s->%s point_time=%s imu_acc_scale=%.6f",
            ...
            m_node_config.require_point_time ? "required" : "optional", m_node_config.imu_acc_scale);
    }
...
private:
    void loadParameters()
    {
        ...
        if (m_node_config.imu_topic.empty() || m_node_config.lidar_topic.empty() ||
            ...
            m_node_config.world_frame.empty())
        {
    ...
    void lidarCB(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
    {
        ...
        PointCloudConversionResult converted = Utils::ouster2PCL(
        ...
        if (!converted.success)
        {
            RCLCPP_WARN_THROTTLE(
                get_logger(), *get_clock(), 2000,
                "Dropping invalid Ouster cloud: %s", converted.error.c_str());
            return;
        }
        ...
        m_state_data.lidar_buffer.emplace_back(timestamp, converted.cloud);
        m_state_data.last_lidar_time = timestamp;
    }
    ...
    bool syncPackage()
    {
        ...
        if (!m_state_data.lidar_pushed)
        {
            ...
            m_package.cloud_end_time =
                m_package.cloud_start_time + static_cast<double>(m_package.cloud->points.back().curvature) * 1e-3;
